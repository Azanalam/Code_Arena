      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {filtered.map((p) => (
              <div key={p.id} className="flex items-center gap-4 bg-zinc-900 rounded-lg px-5 py-4 border border-zinc-800 hover:border-zinc-700 transition-colors">
                ...
                <span className="text-xs text-zinc-600 uppercase">{p.category}</span>
                <button onClick={() => startPractice(p.slug)} className="text-xs px-4 py-1.5 bg-blue-600 hover:bg-blue-500 rounded-md font-medium transition-all">
                  Practice
                </button>
              </div>
            ))}
